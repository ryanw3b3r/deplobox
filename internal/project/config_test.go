...
func TestValidateProjectConfig_MissingGitDirectory(t *testing.T) {
	...
	errors := ValidateProjectConfig("test-project", config)
	if len(errors) == 0 {
		t.Error("Expected missing .git directory to be rejected")
	}

	found := false
	for _, err := range errors {
		if strings.Contains(err, "not a git repository") || strings.Contains(err, "missing .git") {
			found = true
			break
		}
	}
	if !found {
		t.Errorf("Expected 'not a git repository' error, got: %v", errors)
	}
}
