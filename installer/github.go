...
func uploadDeployKey(c *Config) error {
	...
	for _, key := range keys {
		if key.Key != nil && strings.TrimSpace(*key.Key) == pubKeyContent {
			fmt.Printf("%-70s%s[OK]%s\n", "Deploy key already exists on GitHub...", colorGreen, colorReset)
			return nil
		}
	}
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "key is already in use") {
			...
			fmt.Printf("  Deploy key may already exist\n")
			return nil
		}
		fmt.Printf("%s[FAIL]%s\n", colorRed, colorReset)
	...
	for _, hook := range hooks {
		if hook.Config != nil {
			if url, ok := hook.Config["url"].(string); ok && url == webhookURL {
				fmt.Printf("%-70s%s[OK]%s\n", "Webhook already exists on GitHub...", colorGreen, colorReset)
				return nil
			}
		}
	}
